...
class BaseBot:
    ...
    def train_one_step(self, input_tensors, target):
        ...
        batch_loss = self.criterion(self.extract_prediction(output)[0], target)
        batch_loss.backward()
        ...
        with torch.set_grad_enabled(False):
            for *input_tensors, y_local in tqdm(loader):
                ...
                batch_loss = self.criterion(
                    self.extract_prediction(output)[0], y_local.to(self.device))
    ...
    def predict(self, loader, *, return_y=False):
        ...
        with torch.set_grad_enabled(False):
            for *input_tensors, y_local in tqdm(loader):
                input_tensors = [x.to(self.device) for x in input_tensors]
                outputs.append(self.predict_batch(input_tensors)[0].cpu())
